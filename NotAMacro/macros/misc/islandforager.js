...
export default class IslandForager extends Macro {
    ...
    setYaw(yaw) {
        if (yaw > 180 || yaw < -180 || !(yaw < 180 && yaw > -180) || yaw == NaN) {
            ChatLib.chat(`§cAttmpeted to set yaw to ${yaw} but yaw must be between -180 and 180 (ignored update)`);
            if (Config.IslandForagerStopOnError){
                this.tripped = true;
    ...
    setPitch(pitch) {
        if (pitch > 90 || pitch < -90 || !(pitch < 90 && pitch > -90) || pitch == NaN) {
            ChatLib.chat(`§cAttmpeted to set pitch to ${pitch} but pitch must be between -90 and 90 (ignored update)`);
            if (Config.IslandForagerStopOnError){
                this.tripped = true;
    ...
    tickPitchAndYaw() {
        if (this.yawTarget != Player.getYaw()) {
            ...
            else if (this.rollingYawRand < -0.5)
                this.rollingYawRand = -0.5;
        ...
        if (this.pitchTarget != Player.getPitch()) {
            ...
            else if (this.rollingPitchRand < -0.5)
                this.rollingPitchRand = -0.5;
    ...
    setSlot(slot) {
        if (Player.getHeldItemIndex() != slot) {
            if (!Safety.setSlot(slot) && Config.IslandForagerStopOnError) {
                this.tripped = true;
    ...
    getTreeCount() {
        ...
        for (let x = -2; x < 3; x++) {
            for (let z = -2; z < 3; z++) {
                if (World.getBlockAt(Player.getX() + x, Player.getY(), Player.getZ() + z).type.getName() == "Oak Sapling")
                  saplingCount++;
            }
        }
    ...
    tick() {
        ...
        if (this.stage == STAGE_WAITING_FOR_BREAK || this.stage == STAGE_WAITING_FOR_GROWTH || this.stage == STAGE_WAITING_FOR_PLACE) {
            ...
            for (let i = 0; i < blocks.length; i++) {
                if (blocks[i].type.getName() == "Oak Sapling")
                    saplingCount++;
                else if (blocks[i].type.getName() == "Wood")
                    woodCount++;
            }
